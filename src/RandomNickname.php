...
class RandomNickname
{
     ...
     * 随机昵称 形容词
     */
    const nicknameTou = ['迷你的','鲜艳的','飞快的','真实的','清新的','幸福的','可耐的','快乐的','冷静的','醉熏的','潇洒的','糊涂的','积极的','冷酷的','深情的','粗暴的',
     ...
     * 名词
     */
    const nicknameWei = ['嚓茶','皮皮虾','皮卡丘','马里奥','小霸王','凉面','便当','毛豆','花生','可乐','灯泡','哈密瓜','野狼','背包','眼神','缘分','雪碧','人生','牛排',
    ...
    {
        $TouNum = rand(0, count(self::nicknameTou) -1);
        $weiNum = rand(0, count(self::nicknameWei) -1);
        $nickname = self::nicknameTou[$TouNum] . self::nicknameWei[$weiNum];
    ...
    public static function getCompellation($quantity ='rand')
    {
        ...
        $nickname = self::arrXing[mt_rand(0,count(self::arrXing)-1)];
        if ($quantity =='rand') {
            $nickname .= mt_rand(0,1) ? self::arrMing[mt_rand(0,count(self::arrMing)-1)] : (mt_rand(0,1) ? self::arrMing[mt_rand(0,count(self::arrMing)-1)] : self::arrMing[mt_rand(0,count(self::arrMing)-1)]) .(mt_rand(0,1) ? self::arrMing[mt_rand(0,count(self::arrMing)-1)] : self::arrMing[mt_rand(0,count(self::arrMing)-1)]);
        } else if($quantity >= 1 || $quantity < 5) {
            for ($i=1;$i<=$quantity;$i++) {
                $nickname .=(mt_rand(0,1)?self::arrMing[mt_rand(0,count(self::arrMing)-1)]:self::arrMing[mt_rand(0,count(self::arrMing)-1)]);
            }
        }
